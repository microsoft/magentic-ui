...
class SentinelBenchAnalyzer:
    ...
    def filter_tasks(self, tasks: List[str]) -> List[str]:
        ...
        logger.info(
            f"Filtered tasks: {len(filtered_tasks)}/{len(tasks)} tasks selected"
        )
    ...
    def setup_plot_style(self):
        ...
        plt.rcParams["axes.prop_cycle"] = plt.cycler(
            "color", ["#9D7FE0", "#B19FE8", "#C5BFF0", "#E8E3F8", "#F5F3FC"]
        )
    ...
    def calculate_cost(
        ...
        if cost_model not in MODEL_PRICING:
            logger.warning(
                f"Unknown model: {cost_model}. Using gpt-4o pricing as default."
            )
            cost_model = "gpt-4o"
    ...
    def check_run_validity(
        self, run_dir: Path, task_name: str, dimension: int, expected_password: str
        ...
        try:
            ...
            if self.config.get("check_messages", False):
                ...
                messages_files = list(
                    task_dir.glob(f"{task_name}_{dimension}_messages.json")
                )
                if messages_files:
                    with open(messages_files[0], "r") as f:
                        messages_content = f.read().upper()
                        expected_full_password = (
                            f"{expected_password}_{dimension}".upper()
                        )
                        has_correct_password = (
                            expected_full_password in messages_content
                        )
                else:
    ...
    def load_task_data(self, run_dir: str, task_name: str) -> pd.DataFrame:
        ...
        if not task_dirs:
            raise ValueError(
                f"No directories found for task '{task_name}' in {run_dir}"
            )
        ...
        for task_dir in task_dirs:
            for dim_dir in task_dir.iterdir():
                ...
                # Load answer data
                answer_files = list(
                    dim_dir.glob(f"{task_name}_{dimension}_answer.json")
                )
                if answer_files:
                    with open(answer_files[0]) as f:
                        answer_data = json.load(f)
                ...
                if self.config.get("require_success", False):
                    if task_data.get("score", 0) < self.config.get(
                        "score_threshold", 1.0
                    ):
                        continue
        ...
        logger.info(
            f"Loaded data for {len(df)} dimension variants of task '{task_name}'"
        )
        return df
    ...
    def save_plot(self, fig: Any, base_path: str, suffix: str = ""):
        ...
        if plot_format in ["png", "both"]:
            fig.savefig(
                plot_path, dpi=self.config.get("plot_dpi", 300), bbox_inches="tight"
            )

        if plot_format in ["pdf", "both"]:
            pdf_path = plot_path.replace(".png", ".pdf")
            fig.savefig(
                pdf_path, dpi=self.config.get("plot_dpi", 300), bbox_inches="tight"
            )
    ...
    def analyze_missing_runs(self):
        ...
        for directory in directories:
            ...
            for task_name in expected_tasks:
                ...
                if self.config.get("custom_dimensions"):
                    expected_dims = [
                        int(d) for d in self.config["custom_dimensions"].split(",")
                    ]
            ...
            completion_rate = (
                (dir_found / dir_expected * 100) if dir_expected > 0 else 0
            )
            ...
            print(
                f"Runs with timeouts: {dir_timeouts} ({timeout_rate:.1f}% of found runs)"
            )
        ...
        if self.config.get("save_csv", False):
            summary_path = os.path.join(
                self.config["output_dir"], "missing_runs_summary.json"
            )
            summary_data = {
    ...
    def analyze_single_task(self, task_name: str):
        ...
        if self.config.get("save_csv", False):
            csv_path = os.path.join(
                self.config["output_dir"], f"{task_name.replace('/', '_')}_analysis.csv"
            )
        ...
        if not self.config.get("no_plots", False):
            base_path = os.path.join(
                self.config["output_dir"], f"{task_name.replace('/', '_')}.png"
            )
    ...
    def analyze_comparison(self):
        ...
        logger.info(
            f"⚖️ Starting comparison analysis for {len(directories)} directories..."
        )
        ...
        directory_paths = [
            os.path.join(self.config["base_path"], dir_name) for dir_name in directories
        ]
        ...
        for task in expected_tasks:
            ...
            for i, (dir_path, label) in enumerate(zip(directory_paths, labels)):
                try:
                    ...
                    logger.warning(
                        f"Failed to load {task} from {label} ({directories[i]}): {e}"
                    )
                    task_data[i] = None
            ...
            if len(valid_dataframes) >= 2:
                ...
                if len(aligned_dfs) > 1:
                    ...
                    for i, df in enumerate(aligned_dfs[1:], 1):
                        base_df, aligned_dfs[i] = self.align_dimensions(
                            base_df, df, task
                        )
        ...
        if len(valid_data) >= 2:
            ...
            if self.config.get("save_combined_plots", True) and not self.config.get(
                "no_plots", False
            ):
        ...
        if self.config.get("task_type_analysis", False) and len(valid_data) >= 2:
            first_dirs = list(valid_data.keys())[:2]
            self.analyze_task_types_comparison(
                valid_data[first_dirs[0]], valid_data[first_dirs[1]]
            )
    ...
    def analyze_task_types_single(self, df: pd.DataFrame):
        ...
        for task_type in ["duration", "count"]:
            ...
            if not self.config.get("no_plots", False):
                base_path = os.path.join(
                    self.config["output_dir"], f"task_type_{task_type}.png"
                )
    ...
    def analyze_task_types_comparison(
        self, main_df: pd.DataFrame, compare_df: pd.DataFrame
    ):
        ...
        for task_type in ["duration", "count"]:
            ...
            if not self.config.get("no_plots", False):
                base_path = os.path.join(
                    self.config["output_dir"], f"task_type_comparison_{task_type}.png"
                )
    ...
    def align_dimensions(
        self, main_df: pd.DataFrame, compare_df: pd.DataFrame, task_name: str
    ...
    def create_combined_comparison_plot(
        ...
        main_agg = (
            main_df.groupby("dimension")
            .agg({"score": "mean", "duration": "mean", "cost": "mean"})
            .reset_index()
        )

        compare_agg = (
            compare_df.groupby("dimension")
            .agg({"score": "mean", "duration": "mean", "cost": "mean"})
            .reset_index()
        )
        ...
        # Prepare data
        main_acc = [
            main_agg[main_agg["dimension"] == d]["score"].iloc[0] * 100
            for d in common_dims
        ]
        compare_acc = [
            compare_agg[compare_agg["dimension"] == d]["score"].iloc[0] * 100
            for d in common_dims
        ]

        main_dur = [
            main_agg[main_agg["dimension"] == d]["duration"].iloc[0] / 60
            for d in common_dims
        ]
        compare_dur = [
            compare_agg[compare_agg["dimension"] == d]["duration"].iloc[0] / 60
            for d in common_dims
        ]

        main_cost = [
            main_agg[main_agg["dimension"] == d]["cost"].iloc[0] for d in common_dims
        ]
        compare_cost = [
            compare_agg[compare_agg["dimension"] == d]["cost"].iloc[0]
            for d in common_dims
        ]
        ...
        # Overall title
        fig.suptitle(
            "Performance Comparison Analysis", fontsize=22, fontweight="bold", y=1.05
        )
    ...
    def create_accuracy_plot(self, df: pd.DataFrame, task_name: str):
        ...
        ax.set_title(
            f'Success Rate: {task_name.replace("-", " ").title()}', fontweight="bold"
        )
        ax.set_ylim(0, 105)
    ...
    def create_latency_plot(self, df: pd.DataFrame, task_name: str):
        ...
        ax.set_title(
            f'Duration: {task_name.replace("-", " ").title()}', fontweight="bold"
        )
        ax.set_xticks(x_positions)
        ax.set_xticklabels(dimensions)
    ...
    def create_cost_plot(self, df: pd.DataFrame, task_name: str):
        ...
        ax.set_title(
            f'Cost: {task_name.replace("-", " ").title()} ({self.config["model"]})',
            fontweight="bold",
        )
        ax.set_xticks(x_positions)
        ax.set_xticklabels(dimensions)
    ...
    def create_task_type_plot(self, df: pd.DataFrame, task_type: str):
        ...
        # agg_data unused, but keeping aggregation logic for potential future use
        df.groupby("dimension").agg(
            {"score": "mean", "duration": "mean", "cost": "mean"}
        ).reset_index()

        return self.create_combined_single_task_plot(
            df, f"{task_type.title()}-Based Tasks"
        )

    def create_task_type_comparison_plot(
        self, main_df: pd.DataFrame, compare_df: pd.DataFrame, task_type: str
    ):
    ...
    def print_single_task_summary(self, df: pd.DataFrame, task_name: str):
        ...
        for dim in dimensions:
            ...
            print(
                f"{dim:<10} {success_rate:<11.1f}% {avg_duration:<14.1f}m ${avg_cost:<9.3f}"
            )
    ...
    def print_directory_summary(self, df: pd.DataFrame, successful_tasks: List[str]):
        ...
        print(f"Total runs: {len(df)}")
        print(
            f"Tasks: {', '.join(successful_tasks[:10])}{'...' if len(successful_tasks) > 10 else ''}"
        )
        ...
        task_summary = (
            df.groupby("task_id")
            .agg({"score": "mean", "duration": "mean", "cost": "sum"})
            .round(3)
        )
        ...
        for task, row in top_tasks.iterrows():
            print(
                f"  {task}: {row['success_rate']:.1f}% success, {row['duration_min']:.1f}m avg, ${row['cost']:.3f} total"
            )
    ...
    def print_comparison_summary(
        ...
        print(
            f"Runs: {self.config['labels'][0]}: {len(main_df)}, {self.config['labels'][1]}: {len(compare_df)}"
        )
        ...
        print("-" * 80)
        print(
            f"{'Success Rate':<20} {main_success:<19.1f}% {compare_success:<19.1f}% {success_diff:+.1f}%"
        )
        print(
            f"{'Avg Duration':<20} {main_duration:<19.1f}m {compare_duration:<19.1f}m {duration_diff:+.1f}m"
        )
        print(
            f"{'Avg Cost':<20} ${main_cost:<18.3f} ${compare_cost:<18.3f} ${cost_diff:+.3f}"
        )
        ...
        if success_diff > 0:
            print(
                f"   Success: {self.config['labels'][0]} wins by {success_diff:.1f} percentage points"
            )
        elif success_diff < 0:
            print(
                f"   Success: {self.config['labels'][1]} wins by {abs(success_diff):.1f} percentage points"
            )
        else:
            print("   Success: Tie")
        ...
        if duration_diff < 0:
            print(
                f"   Speed: {self.config['labels'][0]} is faster by {abs(duration_diff):.1f} minutes"
            )
        elif duration_diff > 0:
            print(
                f"   Speed: {self.config['labels'][1]} is faster by {duration_diff:.1f} minutes"
            )
        else:
            print("   Speed: Tie")
        ...
        if cost_diff < 0:
            print(
                f"   Cost: {self.config['labels'][0]} is cheaper by ${abs(cost_diff):.3f}"
            )
        elif cost_diff > 0:
    ...
    def print_task_type_comparison_summary(
        self, main_df: pd.DataFrame, compare_df: pd.DataFrame, task_type: str
    ):
        ...
        print(
            f"  Success: {self.config['labels'][0]}: {main_success:.1f}%, {self.config['labels'][1]}: {compare_success:.1f}% (diff: {success_diff:+.1f}%)"
        )
        print(
            f"  Duration: {self.config['labels'][0]}: {main_duration:.1f}m, {self.config['labels'][1]}: {compare_duration:.1f}m (diff: {duration_diff:+.1f}m)"
        )
        print(
            f"  Cost: {self.config['labels'][0]}: ${main_cost:.3f}, {self.config['labels'][1]}: ${compare_cost:.3f} (diff: ${cost_diff:+.3f})"
        )
...
def main(
    ...
    # === QUICK START OPTIONS ===
    quick_check: Annotated[
        bool,
        typer.Option(
            help="⚡ Fast missing runs check only", rich_help_panel="🚀 Quick Start"
        ),
    ] = False,
    full_comparison: Annotated[
        bool,
        typer.Option(
    ...
    single_comprehensive: Annotated[
        bool,
        typer.Option(
    ...
    # === ANALYSIS TYPES ===
    single_task_analysis: Annotated[
        bool,
        typer.Option(
    ...
    full_directory_analysis: Annotated[
        bool,
        typer.Option(
    ...
    save_individual_plots: Annotated[
        bool,
        typer.Option(
    ...
    **Quick Examples:**
    ...
    if model not in MODEL_PRICING:
        typer.echo(
            f"❌ Invalid model: {model}. Must be one of: {', '.join(MODEL_PRICING.keys())}",
            err=True,
        )
        raise typer.Exit(1)
    ...
    analysis_modes = [
        single_task_analysis,
        full_directory_analysis,
        comparison_analysis,
        missing_runs_check,
        task_type_analysis,
    ]
    ...
    if not any(analysis_modes):
        typer.echo(
            "❌ No analysis mode selected. Use --help to see available options.",
            err=True,
        )
        raise typer.Exit(1)
    ...
    if label_names:
        ...
        if len(labels_list) != len(directories_list):
            typer.echo(
                f"❌ Number of labels ({len(labels_list)}) must match number of directories ({len(directories_list)})",
                err=True,
            )
            raise typer.Exit(1)
    else:
