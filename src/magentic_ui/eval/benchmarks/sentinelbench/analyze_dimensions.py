...
def plot_accuracy_vs_dimension(df: pd.DataFrame, task_name: str, sentinel: bool = False, save_path: Optional[str] = None):
    ...
    ax.set_title(f'Success Rate vs Dimension: {task_name.replace("-", " ").title()}{sentinel_suffix}', fontweight='bold')
    ax.set_ylim(0, 105)
...
def plot_accuracy_comparison(df_main: pd.DataFrame, df_compare: pd.DataFrame, task_name: str, 
                           main_label: str, compare_label: str, save_path: Optional[str] = None):
    ...
    dimensions = sorted(set(accuracy_main["dimension"]) & set(accuracy_compare["dimension"]))
    ...
    for dim in dimensions:
        ...
        main_rates.append(main_row.iloc[0]["accuracy_rate"] * 100 if len(main_row) > 0 else 0)
        compare_rates.append(compare_row.iloc[0]["accuracy_rate"] * 100 if len(compare_row) > 0 else 0)
    
    # Create grouped bar chart
    x = np.arange(len(dimensions))
    width = 0.35
    
    bars1 = ax.bar(x - width/2, main_rates, width, label=main_label, 
                   color='#B19FE8', alpha=0.85, edgecolor='#7B68C7', linewidth=1.5)
    bars2 = ax.bar(x + width/2, compare_rates, width, label=compare_label,
                   color='#7B68C7', alpha=0.85, edgecolor='#5A4B7B', linewidth=1.5)
    
    # Add value labels on bars
    for i, (bar1, bar2, rate1, rate2) in enumerate(zip(bars1, bars2, main_rates, compare_rates)):
        height1 = bar1.get_height()
        height2 = bar2.get_height()
        
        ax.text(bar1.get_x() + bar1.get_width()/2, height1 + 1,
                f'{rate1:.1f}%', ha='center', va='bottom', 
                fontweight='bold', fontsize=10, color='#5A4B7B')
        ax.text(bar2.get_x() + bar2.get_width()/2, height2 + 1,
                f'{rate2:.1f}%', ha='center', va='bottom', 
                fontweight='bold', fontsize=10, color='#5A4B7B')
    
    ax.set_xlabel('Task Dimension', fontweight='bold')
    ax.set_ylabel('Success Rate (%)', fontweight='bold')
    ax.set_title(f'Success Rate Comparison: {task_name.replace("-", " ").title()}', fontweight='bold')
    ax.set_ylim(0, 105)
    ax.yaxis.set_major_formatter(PercentFormatter())
...
def plot_latency_vs_dimension(df: pd.DataFrame, task_name: str, sentinel: bool = False, save_path: Optional[str] = None):
    ...
    ax.set_title(f'Task Duration vs Dimension: {task_name.replace("-", " ").title()}{sentinel_suffix}', fontweight='bold')
...
def plot_latency_comparison(df_main: pd.DataFrame, df_compare: pd.DataFrame, task_name: str,
                          main_label: str, compare_label: str, save_path: Optional[str] = None):
    """Create latency comparison plot with grouped bars."""
    setup_plot_style()
    
    # Calculate latency data for both datasets
    latency_main = df_main.groupby("dimension").agg({
        "duration": ["mean", "std", "count"]
    }).round(2)
    ...
    latency_compare = df_compare.groupby("dimension").agg({
        "duration": ["mean", "std", "count"]
    }).round(2)
    ...
    # Get common dimensions
    dimensions = sorted(set(latency_main["dimension"]) & set(latency_compare["dimension"]))
    ...
    for dim in dimensions:
        ...
        main_latencies.append(main_row.iloc[0]["mean_duration_min"] if len(main_row) > 0 else 0)
        compare_latencies.append(compare_row.iloc[0]["mean_duration_min"] if len(compare_row) > 0 else 0)
        main_stds.append(main_row.iloc[0]["std_duration_min"] if len(main_row) > 0 else 0)
        compare_stds.append(compare_row.iloc[0]["std_duration_min"] if len(compare_row) > 0 else 0)
    
    # Create grouped bar chart
    x = np.arange(len(dimensions))
    width = 0.35
    
    bars1 = ax.bar(x - width/2, main_latencies, width, 
                   yerr=main_stds, capsize=4, label=main_label,
                   color='#B19FE8', alpha=0.85, edgecolor='#7B68C7', linewidth=1.5,
                   error_kw={'color': '#5A4B7B', 'linewidth': 1.5})
    bars2 = ax.bar(x + width/2, compare_latencies, width,
                   yerr=compare_stds, capsize=4, label=compare_label,
                   color='#7B68C7', alpha=0.85, edgecolor='#5A4B7B', linewidth=1.5,
                   error_kw={'color': '#5A4B7B', 'linewidth': 1.5})
    
    # Add value labels on bars
    max_std = max(max(main_stds), max(compare_stds))
    for i, (bar1, bar2, lat1, lat2) in enumerate(zip(bars1, bars2, main_latencies, compare_latencies)):
        height1 = bar1.get_height()
        height2 = bar2.get_height()
        
        ax.text(bar1.get_x() + bar1.get_width()/2, height1 + max_std * 0.1,
                f'{lat1:.1f}m', ha='center', va='bottom', 
                fontweight='bold', fontsize=10, color='#5A4B7B')
        ax.text(bar2.get_x() + bar2.get_width()/2, height2 + max_std * 0.1,
                f'{lat2:.1f}m', ha='center', va='bottom', 
                fontweight='bold', fontsize=10, color='#5A4B7B')
    
    ax.set_xlabel('Task Dimension', fontweight='bold')
    ax.set_ylabel('Average Duration (minutes)', fontweight='bold')
    ax.set_title(f'Duration Comparison: {task_name.replace("-", " ").title()}', fontweight='bold')
...
def plot_cost_vs_dimension(df: pd.DataFrame, task_name: str, model: str, sentinel: bool = False, save_path: Optional[str] = None):
    ...
    ax.set_title(f'Task Cost vs Dimension: {task_name.replace("-", " ").title()}{sentinel_suffix} ({model})', fontweight='bold')
...
def plot_cost_comparison(df_main: pd.DataFrame, df_compare: pd.DataFrame, task_name: str, model: str,
                        main_label: str, compare_label: str, save_path: Optional[str] = None):
    """Create cost comparison plot with grouped bars."""
    setup_plot_style()
    
    # Calculate costs for both datasets
    df_main["cost"] = df_main.apply(lambda row: calculate_cost(row["prompt_tokens"], row["completion_tokens"], model), axis=1)
    df_compare["cost"] = df_compare.apply(lambda row: calculate_cost(row["prompt_tokens"], row["completion_tokens"], model), axis=1)
    
    # Group by dimension and calculate mean cost
    cost_main = df_main.groupby("dimension").agg({
        "cost": ["mean", "std", "count"]
    }).round(4)
    cost_main.columns = ["mean_cost", "std_cost", "count"]
    cost_main = cost_main.reset_index()
    
    cost_compare = df_compare.groupby("dimension").agg({
        "cost": ["mean", "std", "count"]
    }).round(4)
    ...
    for dim in dimensions:
        ...
        compare_costs.append(compare_row.iloc[0]["mean_cost"] if len(compare_row) > 0 else 0)
        main_stds.append(main_row.iloc[0]["std_cost"] if len(main_row) > 0 else 0)
        compare_stds.append(compare_row.iloc[0]["std_cost"] if len(compare_row) > 0 else 0)
    
    # Create grouped bar chart
    x = np.arange(len(dimensions))
    width = 0.35
    
    bars1 = ax.bar(x - width/2, main_costs, width,
                   yerr=main_stds, capsize=4, label=main_label,
                   color='#B19FE8', alpha=0.85, edgecolor='#7B68C7', linewidth=1.5,
                   error_kw={'color': '#5A4B7B', 'linewidth': 1.5})
    bars2 = ax.bar(x + width/2, compare_costs, width,
                   yerr=compare_stds, capsize=4, label=compare_label,
                   color='#7B68C7', alpha=0.85, edgecolor='#5A4B7B', linewidth=1.5,
                   error_kw={'color': '#5A4B7B', 'linewidth': 1.5})
    ...
    # Add value labels on bars
    max_std = max(max(main_stds), max(compare_stds))
    for i, (bar1, bar2, cost1, cost2) in enumerate(zip(bars1, bars2, main_costs, compare_costs)):
        height1 = bar1.get_height()
        height2 = bar2.get_height()
        
        ax.text(bar1.get_x() + bar1.get_width()/2, height1 + max_std * 0.1,
                f'${cost1:.3f}', ha='center', va='bottom', 
                fontweight='bold', fontsize=10, color='#5A4B7B')
        ax.text(bar2.get_x() + bar2.get_width()/2, height2 + max_std * 0.1,
                f'${cost2:.3f}', ha='center', va='bottom', 
                fontweight='bold', fontsize=10, color='#5A4B7B')
    
    ax.set_xlabel('Task Dimension', fontweight='bold')
    ax.set_ylabel('Average Cost (USD)', fontweight='bold')
    ax.set_title(f'Cost Comparison: {task_name.replace("-", " ").title()} ({model})', fontweight='bold')
...
def create_combined_plot(df: pd.DataFrame, task_name: str, model: str, sentinel: bool = False, save_path: Optional[str] = None):
    ...
    sentinel_suffix = " (With Sentinel)" if sentinel else " (Without Sentinel)"
    fig.suptitle(f'Performance Analysis: {task_name.replace("-", " ").title()}{sentinel_suffix}', 
...
def create_combined_comparison_plot(df_main: pd.DataFrame, df_compare: pd.DataFrame, task_name: str, model: str,
                                  main_label: str, compare_label: str, save_path: Optional[str] = None):
    ...
    for dim in dimensions:
        ...
        main_acc_rates.append(main_row.iloc[0]["accuracy_rate"] * 100 if len(main_row) > 0 else 0)
        compare_acc_rates.append(compare_row.iloc[0]["accuracy_rate"] * 100 if len(compare_row) > 0 else 0)
    
    bars1_1 = ax1.bar(x - width/2, main_acc_rates, width, label=main_label,
                     color='#B19FE8', alpha=0.85, edgecolor='#7B68C7', linewidth=1.5)
    bars1_2 = ax1.bar(x + width/2, compare_acc_rates, width, label=compare_label,
                     color='#7B68C7', alpha=0.85, edgecolor='#5A4B7B', linewidth=1.5)
    
    for i, (bar1, bar2, acc1, acc2) in enumerate(zip(bars1_1, bars1_2, main_acc_rates, compare_acc_rates)):
        height1 = bar1.get_height()
        height2 = bar2.get_height()
        ax1.text(bar1.get_x() + bar1.get_width()/2, height1 + 1,
                f'{acc1:.1f}%', ha='center', va='bottom', 
                fontsize=9, fontweight='bold', color='#5A4B7B')
        ax1.text(bar2.get_x() + bar2.get_width()/2, height2 + 1,
                f'{acc2:.1f}%', ha='center', va='bottom', 
                fontsize=9, fontweight='bold', color='#5A4B7B')
    ...
    for dim in dimensions:
        ...
        main_latencies.append(main_row.iloc[0]["duration_min"] if len(main_row) > 0 else 0)
        compare_latencies.append(compare_row.iloc[0]["duration_min"] if len(compare_row) > 0 else 0)
    
    bars2_1 = ax2.bar(x - width/2, main_latencies, width, label=main_label,
                     color='#B19FE8', alpha=0.85, edgecolor='#7B68C7', linewidth=1.5)
    bars2_2 = ax2.bar(x + width/2, compare_latencies, width, label=compare_label,
                     color='#7B68C7', alpha=0.85, edgecolor='#5A4B7B', linewidth=1.5)
    
    for i, (bar1, bar2, lat1, lat2) in enumerate(zip(bars2_1, bars2_2, main_latencies, compare_latencies)):
        height1 = bar1.get_height()
        height2 = bar2.get_height()
        ax2.text(bar1.get_x() + bar1.get_width()/2, height1 * 1.02,
                f'{lat1:.1f}m', ha='center', va='bottom', 
                fontsize=9, fontweight='bold', color='#5A4B7B')
        ax2.text(bar2.get_x() + bar2.get_width()/2, height2 * 1.02,
                f'{lat2:.1f}m', ha='center', va='bottom', 
                fontsize=9, fontweight='bold', color='#5A4B7B')
    ...
    bars3_1 = ax3.bar(x - width/2, main_costs, width, label=main_label,
                     color='#B19FE8', alpha=0.85, edgecolor='#7B68C7', linewidth=1.5)
    bars3_2 = ax3.bar(x + width/2, compare_costs, width, label=compare_label,
                     color='#7B68C7', alpha=0.85, edgecolor='#5A4B7B', linewidth=1.5)
    
    for i, (bar1, bar2, cost1, cost2) in enumerate(zip(bars3_1, bars3_2, main_costs, compare_costs)):
        height1 = bar1.get_height()
        height2 = bar2.get_height()
        ax3.text(bar1.get_x() + bar1.get_width()/2, height1 * 1.02,
                f'${cost1:.3f}', ha='center', va='bottom', 
                fontsize=9, fontweight='bold', color='#5A4B7B')
        ax3.text(bar2.get_x() + bar2.get_width()/2, height2 * 1.02,
                f'${cost2:.3f}', ha='center', va='bottom', 
                fontsize=9, fontweight='bold', color='#5A4B7B')
    ...
    # Add elegant overall title
    fig.suptitle(f'Performance Comparison: {task_name.replace("-", " ").title()}', 
                 fontsize=22, fontweight='bold', y=1.05, color='#5A4B7B')
...
def print_summary_statistics(df: pd.DataFrame, task_name: str, model: str, sentinel: bool = False):
...
def print_comparison_summary(df_main: pd.DataFrame, df_compare: pd.DataFrame, task_name: str, model: str,
                           main_label: str, compare_label: str):
    ...
    # Get common dimensions
    dimensions = sorted(set(df_main['dimension'].unique()) & set(df_compare['dimension'].unique()))
    
    print(f"\nDimensions analyzed: {dimensions}")
    print(f"Total runs: {main_label}: {len(df_main)}, {compare_label}: {len(df_compare)}")
    print(f"Model used for cost calculation: {model}")
    ...
    for dim in dimensions:
        ...
        if len(acc_main_row) > 0 and len(acc_compare_row) > 0:
            ...
            print(f"{dim:<10} {'Success Rate':<15} {acc_main:<19.1f}% {acc_compare:<19.1f}% {acc_diff:+.1f}%")
        ...
        print(f"{'':<10} {'Duration':<15} {dur_main:<19.1f}m {dur_compare:<19.1f}m {dur_diff:+.1f}m")
        ...
        print(f"{'':<10} {'Cost':<15} ${cost_main:<18.3f} ${cost_compare:<18.3f} ${cost_diff:+.3f}")
        print("-" * 85)
    ...
    print(f"{'Avg Success Rate':<20} {overall_acc_main:<19.1f}% {overall_acc_compare:<19.1f}% {overall_acc_diff:+.1f}%")
    ...
    print(f"{'Avg Duration':<20} {overall_dur_main:<19.1f}m {overall_dur_compare:<19.1f}m {overall_dur_diff:+.1f}m")
    ...
    print(f"{'Avg Cost':<20} ${overall_cost_main:<18.3f} ${overall_cost_compare:<18.3f} ${overall_cost_diff:+.3f}")
    
    # Total costs
    total_cost_main = df_main['cost'].sum()
    total_cost_compare = df_compare['cost'].sum()
    total_cost_diff = total_cost_main - total_cost_compare
    print(f"{'Total Cost':<20} ${total_cost_main:<18.3f} ${total_cost_compare:<18.3f} ${total_cost_diff:+.3f}")
    
    # Total tokens
    total_tokens_main = df_main['total_tokens'].sum()
    total_tokens_compare = df_compare['total_tokens'].sum()
    print(f"{'Total Tokens':<20} {total_tokens_main:<19,} {total_tokens_compare:<19,} {total_tokens_main - total_tokens_compare:+,}")
    
    # Performance summary
    print(f"\n🏆 Performance Winner Summary:")
    if overall_acc_diff > 0:
        print(f"   Accuracy: {main_label} wins by {overall_acc_diff:.1f} percentage points")
    elif overall_acc_diff < 0:
        print(f"   Accuracy: {compare_label} wins by {abs(overall_acc_diff):.1f} percentage points")
    else:
        print(f"   Accuracy: Tie")
        
    if overall_dur_diff < 0:
        print(f"   Speed: {main_label} wins (faster by {abs(overall_dur_diff):.1f} minutes)")
    elif overall_dur_diff > 0:
        print(f"   Speed: {compare_label} wins (faster by {overall_dur_diff:.1f} minutes)")
    else:
        print(f"   Speed: Tie")
        
    if overall_cost_diff < 0:
        print(f"   Cost Efficiency: {main_label} wins (cheaper by ${abs(overall_cost_diff):.3f})")
    elif overall_cost_diff > 0:
        print(f"   Cost Efficiency: {compare_label} wins (cheaper by ${overall_cost_diff:.3f})")
    else:
        print(f"   Cost Efficiency: Tie")

def main(
    run_dir: Annotated[str, typer.Option(help="Path to run directory (e.g., runs/MagenticUI/SentinelBench/test/4000)")],
    task_name: Annotated[str, typer.Option(help="Name of the task to analyze (e.g., animal-mover-easy)")],
    ...
    output_prefix: Annotated[Optional[str], typer.Option(help="Prefix for output files (default: task-name)")] = None,
    combined: Annotated[bool, typer.Option(help="Create combined plot with all metrics")] = False,
    save_csv: Annotated[bool, typer.Option(help="Save processed data to CSV file")] = False,
    sentinel: Annotated[bool, typer.Option(help="Add sentinel suffix to plots and filenames")] = False,
    compare_with: Annotated[Optional[str], typer.Option(help="Path to second run directory for comparison")] = None,
    main_label: Annotated[str, typer.Option(help="Label for main run in comparison mode")] = "Run 1",
    compare_label: Annotated[str, typer.Option(help="Label for comparison run")] = "Run 2",
):
    # Validate model choice
    if model not in MODEL_PRICING:
        typer.echo(f"❌ Model must be one of: {', '.join(MODEL_PRICING.keys())}", err=True)
        raise typer.Exit(1)
    ...
    if output_prefix is None:
        ...
        if is_comparison:
            ...
            output_prefix = f"{base_prefix}-with-sentinel" if sentinel else f"{base_prefix}-without-sentinel"
    
    try:
        if is_comparison:
            # Comparison mode: load data from both directories
            logger.info(f"Loading data for comparison: '{task_name}' from {run_dir} vs {compare_with}")
            df_main = load_task_data(run_dir, task_name)
            df_compare = load_task_data(compare_with, task_name)
            ...
            if save_csv:
                csv_path_main = os.path.join(output_dir, f"{output_prefix}_{main_label.lower().replace(' ', '_')}_analysis.csv")
                csv_path_compare = os.path.join(output_dir, f"{output_prefix}_{compare_label.lower().replace(' ', '_')}_analysis.csv")
            ...
            if combined:
                ...
                create_combined_comparison_plot(df_main, df_compare, task_name, model, main_label, compare_label, base_path)
            else:
                # Create individual comparison plots
                logger.info("Creating accuracy comparison plot...")
                plot_accuracy_comparison(df_main, df_compare, task_name, main_label, compare_label, base_path)
                
                logger.info("Creating latency comparison plot...")
                plot_latency_comparison(df_main, df_compare, task_name, main_label, compare_label, base_path)
                
                logger.info("Creating cost comparison plot...")
                plot_cost_comparison(df_main, df_compare, task_name, model, main_label, compare_label, base_path)
            
            # Print comparison summary statistics
            print_comparison_summary(df_main, df_compare, task_name, model, main_label, compare_label)
