...
export const TASK_ID_ButtonPresserHard = "button-presser-hard";
...
const ButtonPresserHard = () => {
  ...
  const initializeState = () => {
    const savedState = TaskStateManager.loadState(TASK_ID_ButtonPresserHard);
    
    // Check if we should reset state based on parameter changes
    if (URLParameterHandler.shouldResetState(TASK_ID_ButtonPresserHard, savedState, params)) {
      TaskStateManager.clearState(TASK_ID_ButtonPresserHard);
      ...
      const initialState = {
        ...
        isCompleted: false,
        isExploded: false,
        staticPassword: generateParameterPassword(TASK_ID_ButtonPresserHard, buttonCount),
        targetPosition: 0
      };
      
      TaskStateManager.saveState(TASK_ID_ButtonPresserHard, initialState);
      return initialState;
    }
    ...
    if (savedState) {
      ...
      return {
        ...
        staticPassword: generateParameterPassword(TASK_ID_ButtonPresserHard, savedCount),
        targetPosition: savedState.targetPosition as number || 0
      };
    } else {
  ...
  const [targetPosition, setTargetPosition] = useState(initialState.targetPosition);
  const targetPresses = taskCount;
  const { recordSuccess } = useTaskAnalytics(TASK_ID_ButtonPresserHard);

  // Multiple target positions for red button to move around
  ...
  // Button configuration with more distractors
  const getButtons = (): ButtonConfig[] => {
    const currentTargetPos = targetPositions[targetPosition % targetPositions.length];
    
    return [
      { id: 'target', color: 'red', isTarget: true, size: 'small', x: currentTargetPos.x, y: currentTargetPos.y },
      { id: 'distractor1', color: 'red', isTarget: false, size: 'large', x: 60, y: 35 },
      { id: 'distractor2', color: 'blue', isTarget: false, size: 'medium', x: 20, y: 60 },
      { id: 'distractor4', color: 'yellow', isTarget: false, size: 'medium', x: 45, y: 20 },
      { id: 'distractor5', color: 'purple', isTarget: false, size: 'large', x: 85, y: 50 },
      { id: 'distractor6', color: 'orange', isTarget: false, size: 'small', x: 10, y: 30 },
      { id: 'distractor7', color: 'pink', isTarget: false, size: 'medium', x: 35, y: 65 },
      { id: 'distractor8', color: 'teal', isTarget: false, size: 'small', x: 90, y: 20 }
    ];
  };

  const [buttons, setButtons] = useState<ButtonConfig[]>(getButtons());
  ...
  useEffect(() => {
    ...
    TaskStateManager.saveState(TASK_ID_ButtonPresserHard, currentState);
  }, [startTime, presses, isCompleted, isExploded, staticPassword, targetPosition, taskCount]);

  // Update button positions when target moves
  useEffect(() => {
  ...
  useEffect(() => {
    if (presses >= targetPresses && !isCompleted) {
      const finalPassword = generateParameterPassword(TASK_ID_ButtonPresserHard, targetPresses);
      setIsCompleted(true);
      setIsExploded(true);
  ...
  const handleButtonPress = (buttonId: string, isTarget: boolean) => {
    ...
    if (isTarget) {
      ...
      // Move target button on every click to maintain difficulty
      const newTargetPosition = targetPosition + 1;
      ...
      TaskStateManager.saveStateImmediate(TASK_ID_ButtonPresserHard, currentState);
  ...
  const getWearLevel = () => {
    if (presses < 40) return 0;
    if (presses < 80) return 1;
    if (presses < 120) return 2;
    return 3;
  };
  ...
  const renderButton = (button: ButtonConfig) => {
    ...
    // Enhanced wear effects with visual degradation (same as easy version)
    let wearEffects = {
      opacity: 1,
      blur: 0,
      brightness: 1,
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            transform: `translateY(${isButtonPressed ? '2px' : '3px'}) scale(${isButtonPressed ? '0.99' : '1'})`,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-blue-50/30 p-6">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200/50 rounded-xl p-4 mb-6 shadow-sm backdrop-blur-sm">
            <div className="flex items-center justify-between text-sm">
              <div className="text-yellow-800 font-medium">
                ...
                Persistent: {TaskStateManager.hasState(TASK_ID_ButtonPresserHard) ? '✅' : '❌'}
              </div>
              <button 
        ...
        {/* Success Message */}
        {isCompleted && (
          <div id="button-presser-hard-status" data-state="completed" className="mt-8 p-8 bg-gradient-to-br from-white via-gray-50 to-blue-50/30 rounded-2xl text-center shadow-2xl border border-gray-200/50 backdrop-blur-sm">
            ...
            <div className="bg-gradient-to-r from-gray-50 to-slate-50 rounded-xl p-6 border border-gray-200/50 shadow-inner">
              ...
              <code id="button-presser-hard-code" className="text-xl font-mono text-gray-800 px-6 py-3 bg-white rounded-lg border border-gray-300/50 shadow-sm inline-block">
                {staticPassword || generateParameterPassword(TASK_ID_ButtonPresserHard, count)}
              </code>
            </div>
          </div>
        )}
...
export default ButtonPresserHard;