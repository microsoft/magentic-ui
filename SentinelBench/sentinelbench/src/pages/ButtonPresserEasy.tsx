...
export const TASK_ID_ButtonPresserEasy = "button-presser-easy";

const ButtonPresserEasy = () => {
  // Parse URL parameters manually (don't clean yet)
  const urlParams = new URLSearchParams(window.location.search);
  const count = parseInt(urlParams.get('count') || '10', 10);
  
  // Validate count parameter
  const buttonCount = (count >= 1 && count <= 100000) ? count : 10;
  ...
  const initializeState = () => {
    const savedState = TaskStateManager.loadState(TASK_ID_ButtonPresserEasy);
    
    // Check if we should reset state based on parameter changes
    if (URLParameterHandler.shouldResetState(TASK_ID_ButtonPresserEasy, savedState, params)) {
      TaskStateManager.clearState(TASK_ID_ButtonPresserEasy);
      ...
      const initialState = {
        ...
        isCompleted: false,
        isExploded: false,
        staticPassword: generateParameterPassword(TASK_ID_ButtonPresserEasy, buttonCount)
      };
      
      TaskStateManager.saveState(TASK_ID_ButtonPresserEasy, initialState);
      return initialState;
    }
    ...
    if (savedState) {
      ...
      return {
        ...
        staticPassword: generateParameterPassword(TASK_ID_ButtonPresserEasy, savedCount)
      };
    } else {
  ...
  const { recordSuccess } = useTaskAnalytics(TASK_ID_ButtonPresserEasy);
  ...
  useEffect(() => {
    ...
    TaskStateManager.saveState(TASK_ID_ButtonPresserEasy, currentState);
  }, [startTime, presses, isCompleted, isExploded, staticPassword, taskCount]);

  // Check for completion
  useEffect(() => {
    if (presses >= targetPresses && !isCompleted) {
      const finalPassword = generateParameterPassword(TASK_ID_ButtonPresserEasy, targetPresses);
      setIsCompleted(true);
      setIsExploded(true);
  ...
  const handleButtonPress = () => {
    ...
    TaskStateManager.saveStateImmediate(TASK_ID_ButtonPresserEasy, currentState);
  ...
  const getWearLevel = () => {
    if (presses < 8) return 0;
    if (presses < 16) return 1;
    if (presses < 22) return 2;
    return 3;
  };
  ...
  const renderButton = () => {
    ...
    // Enhanced red button colors with better depth
    const colors = {
      primary: '#ef4444',
      secondary: '#dc2626',
      tertiary: '#b91c1c',
      ...
      >
        ...
        <div
          ...
          style={{
            ...
            transform: `translateY(${isPressed ? '2px' : '3px'}) scale(${isPressed ? '0.99' : '1'})`,
            transition: 'all 0.2s ease-out',
            boxShadow: `
              0 ${isPressed ? '1px' : '2px'} ${isPressed ? '2px' : '4px'} rgba(0,0,0,0.08),
        ...
        <button
          ...
          style={{
            ...
            boxShadow: `
              0 ${isPressed ? '1px' : '3px'} ${isPressed ? '2px' : '6px'} rgba(0,0,0,0.06),
              inset 0 1px 0 rgba(255,255,255,0.6),
              inset 0 -1px 0 rgba(0,0,0,0.08),
              inset 1px 0 0 rgba(255,255,255,0.3),
              inset -1px 0 0 rgba(0,0,0,0.04)
            `,
            transform: `translateY(${isPressed ? '1px' : '0px'}) scale(${isPressed ? '0.98' : '1'})`,
            opacity: wearEffects.opacity,
            filter: `blur(${wearEffects.blur}px) brightness(${wearEffects.brightness})`,
            borderRadius: '50%',
            transition: 'all 0.2s ease-out'
          }}
          data-button-id="target"
        >
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-gray-50 to-blue-50/30 p-6">
      <div className="max-w-6xl mx-auto">
          ...
          <div className="bg-gradient-to-r from-yellow-50 to-amber-50 border border-yellow-200/50 rounded-xl p-4 mb-6 shadow-sm backdrop-blur-sm">
            <div className="flex items-center justify-between text-sm">
              <div className="text-yellow-800 font-medium">
                ...
                Persistent: {TaskStateManager.hasState(TASK_ID_ButtonPresserEasy) ? '✅' : '❌'}
              </div>
              <button 
        ...
        {isCompleted && (
          <div id="button-presser-easy-status" data-state="completed" className="mt-8 p-8 bg-gradient-to-br from-white via-gray-50 to-blue-50/30 rounded-2xl text-center shadow-2xl border border-gray-200/50 backdrop-blur-sm">
            ...
            <div className="bg-gradient-to-r from-gray-50 to-slate-50 rounded-xl p-6 border border-gray-200/50 shadow-inner">
              ...
              <code id="button-presser-easy-code" className="text-xl font-mono text-gray-800 px-6 py-3 bg-white rounded-lg border border-gray-300/50 shadow-sm inline-block">
                {staticPassword || generateParameterPassword(TASK_ID_ButtonPresserEasy, targetPresses)}
              </code>
            </div>
          </div>
        )}
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          @keyframes button-bounce {
            0%, 100% { transform: translate(-50%, -50%) perspective(800px) rotateX(15deg) scale(1); }
            50% { transform: translate(-50%, -50%) perspective(800px) rotateX(15deg) scale(1.1); }
          }
          .button-pressed {
...
export default ButtonPresserEasy;